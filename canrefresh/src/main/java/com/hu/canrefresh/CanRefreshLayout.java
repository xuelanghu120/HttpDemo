...
public class CanRefreshLayout extends FrameLayout {
    ...
    public CanRefreshLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        final TypedArray a = context.obtainStyledAttributes(attrs, com.hu.canrefresh.R.styleable.CanRefreshLayout, defStyleAttr, 0);

        try {
            final int N = a.getIndexCount();
            for (int i = 0; i < N; i++) {
                int attr = a.getIndex(i);
                if (attr == com.hu.canrefresh.R.styleable.CanRefreshLayout_can_enabled_up) {
                    setRefreshEnabled(a.getBoolean(attr, true));

                } else if (attr == com.hu.canrefresh.R.styleable.CanRefreshLayout_can_enabled_down) {
                    setLoadMoreEnabled(a.getBoolean(attr, true));

                } else if (attr == com.hu.canrefresh.R.styleable.CanRefreshLayout_can_style_up) {
                    mHeadStyle = a.getInt(attr, CLASSIC);

                } else if (attr == com.hu.canrefresh.R.styleable.CanRefreshLayout_can_style_down) {
                    mFootStyle = a.getInt(attr, CLASSIC);

                } else if (attr == com.hu.canrefresh.R.styleable.CanRefreshLayout_can_friction) {

                    setFriction(a.getFloat(attr, DEFAULT_FRICTION));

                } else if (attr == com.hu.canrefresh.R.styleable.CanRefreshLayout_can_duration) {

                    mDuration = a.getInt(attr, DEFAULT_DURATION);

                } else if (attr == com.hu.canrefresh.R.styleable.CanRefreshLayout_can_smooth_duration) {

                    mSmoothDuration = a.getInt(attr, DEFAULT_SMOOTH_DURATION);

                } else if (attr == com.hu.canrefresh.R.styleable.CanRefreshLayout_can_smooth_length) {

                    mSmoothLength = a.getInt(attr, DEFAULT_SMOOTH_LENGTH);

                } else if (attr == com.hu.canrefresh.R.styleable.CanRefreshLayout_can_bg_up) {

                    mRefreshBackgroundResource = a.getResourceId(attr, android.R.color.transparent);

                } else if (attr == com.hu.canrefresh.R.styleable.CanRefreshLayout_can_bg_down) {

                    mLoadMoreBackgroundResource = a.getResourceId(attr, android.R.color.transparent);

                } else if (attr == com.hu.canrefresh.R.styleable.CanRefreshLayout_can_is_coo) {
    ...
    protected void onFinishInflate() {
        ...
        if (childCount > 0) {
            mHeaderView = findViewById(com.hu.canrefresh.R.id.can_refresh_header);
            mContentView = findViewById(com.hu.canrefresh.R.id.can_content_view);
            mFooterView = findViewById(com.hu.canrefresh.R.id.can_refresh_footer);
            mScrollView = findViewById(com.hu.canrefresh.R.id.can_scroll_view);
        }
