 ...
 * 
 */
package com.huxin.common.utils.date;

import com.huxin.common.application.Global;
import com.huxin.common.utils.ToastUtils;
...
public class DateUtil {
	 ...
	 * @param duringTime 本地和服务器时间的差值，校准使用
     * @return  相差天数。如果失败则返回-1
     */
	public static int getIntervalDays(Date date, Date otherDate,long duringTime) {
		int num = -1;
		Date dateTmp = StringToDate(DateUtil.getDate(date),
				DateStyle.YYYY_MM_DD);
		Date otherDateTmp = StringToDate(DateUtil.getDate(otherDate),
				DateStyle.YYYY_MM_DD);
		if (dateTmp != null && otherDateTmp != null) {
			long time = Math.abs(dateTmp.getTime() - otherDateTmp.getTime() - duringTime);
			num = (int) (time / (24 * 60 * 60 * 1000));
		}
		return num;
	}
	...
	public static boolean isInTime2(String startTemp,String endTemp) {
		if (startTemp == null || endTemp==null  ) {
			ToastUtils.showShortToast(Global.getContext(),"参数错误");
			return false;
		}
		if (!startTemp.contains(":") ||  !endTemp.contains(":") ) {
			ToastUtils.showShortToast(Global.getContext(),"参数错误");
			return false;
		}
		String curTime = DateUtil.DateToString(new Date(), DateStyle.HH_MM_SS);

		SimpleDateFormat sdf = new SimpleDateFormat("HH:mm:ss");
		try {
			long now = sdf.parse(curTime).getTime();
			long start = sdf.parse(startTemp).getTime();
			long end = sdf.parse(endTemp).getTime();
			if (endTemp.equals("00:00:00")) {
				endTemp = "24:00:00";
			}
			if (end < start) {
				if (now >= end && now < start) {
